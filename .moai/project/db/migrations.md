...
| `20260619160000_spec009_post_purchase/migration.sql` | 2026-06-19T16:00:00Z | SPEC-009 안 A — `payments` 테이블에 `post_id TEXT` 컬럼 추가 + `payments_post_id_fan_user_id_status_idx` 복합 인덱스 생성 + `posts(id)` FK(`ON DELETE SET NULL`) 추가. 기존 컬럼/데이터 비파괴, 추가형. | No (추가형, Low 위험) |

> **SPEC-008 노트 (스키마 변경 없음)**: 프로그램 완료 처리·정산 릴리스·리뷰 기능은
> 기존 `ProgramStatus.COMPLETED`, `PaymentStatus.RELEASED`, `SettlementStatus.RELEASED`
> enum 값과 `reviews` 모델(이미 `init`에 `@@unique([programId, userId])` 포함)을 그대로
> 사용하므로 신규 마이그레이션이 불필요하다. 본 SPEC은 `schema.md`의 `reviews`
> UNIQUE COMPOSITE 인덱스·제약 누락(문서 드리프트)만 보강했다.
...
| `20260619160000_spec009_post_purchase` | Low | `DROP INDEX payments_post_id_fan_user_id_status_idx` → `ALTER TABLE payments DROP CONSTRAINT payments_post_id_fkey` → `ALTER TABLE payments DROP COLUMN post_id` | 해당 컬럼 데이터만 |
