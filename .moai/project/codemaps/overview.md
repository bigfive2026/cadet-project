...
> 멤버십·포스트 접근제어(SPEC-003), 프로그램 CRUD(SPEC-004), 참여 신청·수락/거절·인앱 알림(SPEC-005)까지 구현 완료.
> 핵심 `lib/` 도메인: `auth`, `membership`, `post-access`, `programs`, `applications`(트랜잭션),
> `notifications`, `queries/*`, `validation/*`. REST 라우트는 `src/app/api/**`, UI는 `src/components/**` + `src/app/(app)/**`.
...
_생성일: 2026-06-18 · 상태: 부분 구현 반영(SPEC-005 완료 시점 갱신)_
