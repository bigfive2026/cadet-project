...
> 계약·약관 서명·Mock 결제·정산(SPEC-006), 커뮤니티·멤버/참여자 명단(SPEC-007),
> 프로그램 완료 처리·정산 릴리스·리뷰(SPEC-008), PAID 포스트 단건 구매·접근제어 확장(SPEC-009)까지 구현 완료.
> 핵심 `lib/` 도메인: `auth`, `membership`, `post-access`(PAID 분기 확장 포함), `programs`, `applications`(트랜잭션),
> `notifications`, `community-access`(권한 파생), `reviews`(완료 승인·리뷰·정산 릴리스 트랜잭션),
> `post-purchase`(PAID 포스트 단건 구매·중복 방지 트랜잭션, SPEC-009),
> `queries/*`(reviews 집계 포함), `validation/*`(post-purchase 검증 포함).
> REST 라우트는 `src/app/api/**`(완료 승인·리뷰·커뮤니티 글 CRUD·PAID 포스트 구매 포함), UI는 `src/components/**` + `src/app/(app)/**`.
...
### 종단간 핵심 흐름
```
크리에이터 스튜디오 → 멤버십 → 유료/멤버 전용 포스트(단건 구매 포함)
...
_생성일: 2026-06-18 · 상태: 부분 구현 반영(SPEC-009 완료 시점 갱신)_
